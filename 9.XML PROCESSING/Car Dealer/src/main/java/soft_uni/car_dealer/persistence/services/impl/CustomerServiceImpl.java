...
public class CustomerServiceImpl implements CustomerService {
    ...
    @Override
    public void seedCustomers(CustomerRootSeedDto customerDtos) {
        for (CustomerDto customerDto : customerDtos.getCustomerDtos()) {
    ...
    @Override
    public CustomerInfoRootDto getOrderedCustomers() {
        ...
        CustomerInfoRootDto customerInfoRootDto = new CustomerInfoRootDto();
        customerInfoRootDto.setCustomerInfoDtos(customerInfoDtos);
        return customerInfoRootDto;
    }

    @Override
    public BuyerRootDto getBuyers() {
        BuyerRootDto buyerRootDto = new BuyerRootDto();
        buyerRootDto.setBuyerDtos(this.customerRepository.getCustomersWithCarsBought(10)
                .stream().map(c->{
                    BuyerDto buyer = new BuyerDto();
                    buyer.setFullName(c.getName());
                    buyer.setBoughtCars(c.getSales().size());
                    buyer.setSpentMoney(calculateSpendMoney(c));
                    return buyer;
                })
                .sorted((b1, b2)->{
                    int result = b2.getSpentMoney().compareTo(b1.getSpentMoney());
                    if (result==0){
